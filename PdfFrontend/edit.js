...
async function loadPdf(fileName) {
	...
	if (localStorage.getItem('isUser') == 'yes') {
		...
		try {
			...
			const response = await fetch(`http://localhost:8080/download/${fileName}/${uid}`);
...
window.onload = async function () {
	...
	// }
	try {
		const fileName = "Sample_Scanned_PDF"; // Replace with the actual file name
		receivedValue = fileName;
		...
		pdfList.push({
			name: receivedValue,
			content: await loadPdf(receivedValue)
		});
...
function updateThumbnailOrder() {
	...
	pages = updatedPages;
	console.log(pages)
	addProcess('Change Page Order', {
		previousOrder: previousPages
	});
...
function deletePage(pageNumber) {
	...
	if (indexToDelete !== -1) {
		...
		addProcess('Delete Page', {
			pageNumber: pageNumber
		});
		deletedPagesList.push({
			pageNumber: pageNumber,
			index: indexToDelete
		});
		renderPage();
...
fileInput.addEventListener('change', async function () {
	...
	if (file) {
		...
		if (pdfList.length > 1) {
			...
			const contentBlob = new Blob([pdfList[1].content], {
				type: 'application/pdf'
			});
...
async function extractedText() {
    try {
        const content = await performOCR();
        const modal = document.createElement('div');
        modal.style.position = 'fixed';
        modal.style.top = '0';
        modal.style.left = '0';
        ...
        // Create a container for the rounded rectangle
        const container = document.createElement('div');
        container.style.backgroundColor = '#FFFFFF'; // White color
        container.style.borderRadius = '10px'; // Rounded corners
        container.style.padding = '20px'; // Padding inside the rectangle
        container.style.width = '80%'; // Adjust the width as needed
        container.style.maxHeight = '80%'; // Maximum height for scrolling
        container.style.overflowY = 'auto'; // Enable vertical scrolling

        // Create a paragraph element for the text content
        const textElement = document.createElement('p');
        textElement.innerHTML = content.replace(/\n/g, '<br>'); // Set the text content with line breaks
        ...
        modal.addEventListener('click', (event) => {
            // Check if the clicked target is outside the white container
...
async function performOCR() {
    ...
    const endpointUrl = 'http://localhost:8080/performOCR'; // Replace with your server URL
